...
public class InstanceReplicaGroupPartitionSelector {
  ...
  public void selectInstances(Map<Integer, List<InstanceConfig>> poolToInstanceConfigsMap,
    ...
    if (_replicaGroupPartitionConfig.isReplicaGroupBased()) {
      ...
      if (_replicaGroupPartitionConfig.isMinimizeDataMovement() && _existingInstancePartitions != null) {
        ...
        Map<Integer, Set<String>> replicaGroupIdToInstancesMap = new HashMap<>();
        for (int replicaGroupId = 0; replicaGroupId < existingNumReplicaGroups; replicaGroupId++) {
          ...
          for (int partitionId = 0; partitionId < existingNumPartitions; partitionId++) {
            ...
            replicaGroupIdToInstancesMap.computeIfAbsent(replicaGroupId, k -> new HashSet<>())
        ...
        for (int replicaGroupId = 0; replicaGroupId < existingNumReplicaGroups; replicaGroupId++) {
          ...
          // Filter out instances that belong to other replica groups which should not be the candidate.
          Set<String> candidateInstances = new LinkedHashSet<>(poolToCandidateInstancesMap.get(pool));
   ...
   */
  protected List<String> getInstancesWithMinimumMovement(int numInstancesToSelect, Set<String> candidateInstances,
    ...
    for (int i = 0; i < existingInstances.size() && i < numInstancesToSelect; i++) {
      ...
      if (candidateInstances.contains(existingInstance)) {
        ...
        // If the instance no longer exists, pick a new instance to fill its vacant position.
        String newInstance = newlyAddedInstances.pollFirst();
        instancesToSelect.set(i, newInstance);
      }
    }

    // If the new number of instances per replica group is greater than the previous one.
    for (int i = existingInstances.size(); i < numInstancesToSelect; i++) {
      if (!newlyAddedInstances.isEmpty()) {
